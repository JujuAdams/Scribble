...
?> You may have seen two different types of linebreak if you've compared text output from Windows and Linux/macOS. Windows uses `\r\n` (`0x0d` `0x0a`) whereas Unix-based systems use `\n` (`0x0a`). The reasons for this are outside the scope of Scribble. Generally `\r` can be ignored as a zero-width whitespace character and `\n` is used as the primary newline character instead.
