function __scribble_garbage_collect()
{
    ...
    repeat(5)
    {
        ...
        global.__scribble_ecache_test_index--;
        
        //Check to see if we need to wrap around
        if (global.__scribble_ecache_test_index < 0)
        {
            //If we do, jump to the end of the list
            global.__scribble_ecache_test_index += array_length(global.__scribble_ecache_array);
            
            //If the size of the list is 0 then we'll still be negative
            if (global.__scribble_ecache_test_index < 0)
            {
                global.__scribble_ecache_test_index = 0; //Clamp to 0
                break;
            }
        }
        
        //Only flush if we want to garbage collect this text element and it hasn't been drawn for a while
        var _element = global.__scribble_ecache_array[global.__scribble_ecache_test_index];
    ...
    repeat(5)
    {
        global.__scribble_mcache_test_index--;
        
        if (global.__scribble_mcache_test_index < 0)
        {
            global.__scribble_mcache_test_index += array_length(global.__scribble_mcache_array);
            if (global.__scribble_mcache_test_index < 0)
            {
        ...
        var _model = global.__scribble_mcache_array[global.__scribble_mcache_test_index];
