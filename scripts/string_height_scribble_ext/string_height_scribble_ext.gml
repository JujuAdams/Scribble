// Feather disable all
/// Emulation of string_height(), but using Scribble for calculating the width
/// 
/// **Please do not use this function in conjunction with string_copy()**
/// 
/// @param string  The string to draw
/// @param width   The maximum width in pixels of the string before a line break

function string_height_scribble_ext(_string, _width)
{
    static _scribbleState = __ScribbleSystem().__state;
    
    var _font = draw_get_font();
    _font = (not font_exists(_font))? _scribbleState.__defaultFont : font_get_name(_font);
    
    return scribble(_string, "__draw_text_ext_scribble__").font(_font).max_size(_width).layout(SCRIBBLE_LAYOUT_WRAP).get_height();
}
