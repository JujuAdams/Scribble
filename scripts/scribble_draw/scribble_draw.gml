...
var _scribble_array = undefined;
var _rebuild        = false;

if (is_array(_draw_string))
{
    _scribble_array = _draw_string;
    ...
    {
        _draw_string = _scribble_array[__SCRIBBLE.STRING];
        _rebuild = true;
        show_debug_message("Scribble: Warning! Rebuilding freed text element (\"" + string(_draw_string) + "\")");
    }
}
else
{
    ...
    if (ds_map_exists(global.__scribble_global_cache_map, _cache_string))
    {
        ...
        _scribble_array = global.__scribble_global_cache_map[? _cache_string];
        if (_scribble_array[__SCRIBBLE.FREED])
        {
            _draw_string = _scribble_array[__SCRIBBLE.STRING];
            _rebuild = true;
            show_debug_message("Scribble: Warning! Rebuilding freed text element (\"" + string(_draw_string) + "\")");
...
if (!is_array(_scribble_array) || _rebuild)
{
    //Cache a new text element if we don't have a relevant one for this string
        
    if (_rebuild)
    {
        var _old_draw_state = scribble_draw_get_state();
        scribble_draw_set_state(_scribble_array[__SCRIBBLE.GLOBAL_DRAW_STATE]);
    ...
    if (!_rebuild)
    {
        _scribble_array = array_create(__SCRIBBLE.__SIZE); //The text element array
    }
    
    var _element_pages_array   = []; //Stores each page of text
    var _glyph_character_array = [];
    ...
    _scribble_array[@ __SCRIBBLE.GLOBAL_DRAW_STATE     ] = scribble_draw_get_state();
    ...
    _scribble_array[@ __SCRIBBLE.GLYPH_ARRAY           ] = _glyph_character_array;
    ...
    _scribble_array[@ __SCRIBBLE.AUTOTYPE_WINDOW_ARRAY ] =  array_create(2*__SCRIBBLE_WINDOW_COUNT, 0.0);
    ...
    repeat(_buffer_size)
    {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if (asset_get_type(_command_name) == asset_sprite)
                                        {
                                            ...
                                            _glyph_character_array[@ array_length_1d(_glyph_character_array)] = _command_name;
        ...
        else if (_character_code == 32) //If we've hit a space
        {
            ...
            ++_meta_element_characters;
            _glyph_character_array[@ array_length_1d(_glyph_character_array)] = _character_code;
    ...
    repeat(array_length_1d(_element_pages_array))
    {
        ...
        {
            var _line_array = _page_lines_array[_l];
    ...
    if (_rebuild)
    {
        scribble_draw_set_state(_old_draw_state);
    }
        ...
        else
        {
            ...
            if ((_typewriter_fade_in >= 0) && (_typewriter_speed > 0))
            {
                ...
                switch(_typewriter_method)
                {
                    ...
                    case SCRIBBLE_AUTOTYPE_PER_LINE:      var _typewriter_count = _page_array[__SCRIBBLE_PAGE.LINES    ] + 1; break;
                }
