...
repeat(ds_list_size(global.__scribble_create_separator_list))
{
    ...
    if (SCRIBBLE_VERBOSE) show_debug_message("Scribble: \"" + _substr + "\"    " + chr(_sep_char));
                ...
                default:
                    ...
                    if (!_found)
                    {
                        ...
                        if (_asset >= 0) && (asset_get_type(global.__scribble_create_parameters_list[| 0]) == asset_sprite)
                        {
                            ...
                            #region Add this sprite to the buffer
                            ...
                            for(var _j = 1; _j < SCRIBBLE_MAX_FLAGS; _j++)
                            {
                                _flags += _text_flags[_j] * _offset;
                                _offset *= 2;
                            }
                            ...
                            for(var _image = _image_a; _image <= _image_b; _image++)
                            {
                                ...
                                if (_sprite_texture != _previous_texture)
                                {
                                    ...
                                    {
                                        var _glyph_buffer = buffer_create(6*__SCRIBBLE_VERTEX_BYTE_SIZE, buffer_grow, 1);
    ...
    else
    {
        ...
        if (_text_font != _previous_font)
        {
            ...
            if (_font_texture != _previous_texture)
            {
                ...
                if (_vbuff_data == undefined)
                {
                    var _glyph_buffer = buffer_create(100*6*__SCRIBBLE_VERTEX_BYTE_SIZE, buffer_grow, 1);
        ...
        var _flags  = 0;
        var _offset = 1;
        for(var _j = 0; _j < SCRIBBLE_MAX_FLAGS; _j++)
        {
            _flags += _offset*_text_flags[_j];
            _offset *= 2;
        }
