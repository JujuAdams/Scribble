...
function __ScribbleDrawCycle(_name)
{
    ...
    with(_data)
    {
        ...
        if (is_array(__data))
        {
            ...
            if (_arrayLength == 1)
            {
                draw_sprite_stretched_ext(sprScribbleFallbackDot, 0, 0, __index, SCRIBBLE_CYCLE_TEXTURE_WIDTH, 1, _array[0], 1);
            }
            else
            {
                if (__smooth)
                {
                    if (__legacyBlend)
                    {
                        ...
                        repeat(SCRIBBLE_CYCLE_TEXTURE_WIDTH)
                        {
                            ...
                            draw_sprite_ext(sprScribbleFallbackDot, 0,
                                            _x, __index,
                                            1, 1, 0,
                    ...
                    else
                    {
                        ...
                        repeat(SCRIBBLE_CYCLE_TEXTURE_WIDTH)
                        {
                            ...
                            draw_sprite_ext(sprScribbleFallbackDot, 0,
                                            _x, __index,
                                            1, 1, 0,
                                            __ScribbleMergeColorOKLab(_array[_indexA], _array[_indexB], _frac), 1);
                            ++_x;
                        }
                    }
                }
